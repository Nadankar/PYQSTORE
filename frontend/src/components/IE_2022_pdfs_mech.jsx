import React from 'react'
import IE_2022_mech from "../Allpdf_json_file/IE_2022_pdfs_mech.json"

export default function IE_2022_pdfs_mech() {
  return (
    <div className="max-w-screen-2xl container mx-auto md:px-20 px-4 mt-20 grid justify-center">
      <div className="overflow-x-auto">
        <table className="table dark:text-white bg-slate-900 ">
          <thead>
            <tr>
              <th></th>
              <th className="text-purple-700 font-medium">{IE_2022_mech.subject}</th>
              <th className="text-purple-700 font-medium">Path</th>
            </tr>
          </thead>
          <tbody>
            {IE_2022_mech.pdfs.map((pdf, index) => (
              <tr key={index}>
                <th>{index + 1}</th>
                <td>{pdf.pdfname}</td>
                <td className="text-blue-400">
                  <a href={pdf.path} target="_blank" rel="noopener noreferrer">
                    {pdf.path}
                  </a>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  )
}
