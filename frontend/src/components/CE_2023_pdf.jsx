...
import ce_2023_pdf from "../Allpdf_json_file/ce_2023_pdf.json"

export default function CE_2023_pdf() {
  return (
    ...
    <div className="overflow-x-auto">
      <table className="table  ">
        <thead>
          <tr>
            <th></th>
            <th className="text-purple-700 font-medium">{ce_2023_pdf.subject}</th>
            <th className="text-purple-700 font-medium">Path</th>
          </tr>
        </thead>
        <tbody>
          {ce_2023_pdf.pdfs.map((pdf, index) => (
            <tr key={index}>
              <th>{index + 1}</th>
              <td>{pdf.pdfname}</td>
